...
public class FormPesananDalamAntrian extends Form{
    ...
    public FormPesananDalamAntrian() {
        init();
    ...
    private JPanel setButton(){
        ...
        JButton btnProses = new JButton("Tandai Proses");
        btnProses.putClientProperty(FlatClientProperties.STYLE, ""
                + "foreground:@accentColor;"
                + "background:rgb(255,255,255);");
        btnProses.setIcon(new FlatSVGIcon("com/radja/icon/edit.svg", 0.4f));
        btnProses.setIconTextGap(5);
        btnProses.addActionListener((e) -> updateData());
        ...
        btnPrint.addActionListener((e) -> printData());
...
private void searchData() {
    ...
    private void printData(){
        ...
        if(row != -1){
            ...
            List<Pesanan> list = servis.getDataById(idPesanan);
            servisReport.generateQRCodeBuktiTransaksi(list);
        }else{
            Toast.show(this, Toast.Type.INFO, "Please select the data you want to print", getOptionAlert());
        }
    }
   ...
   private void updateData() {
    ...
    if (row != -1) {
        Pesanan model = tblModel.getData(row);
        idPesanan = model.getIdPesanan();
        String noPlat = model.getNoPlat();
        String status = "Dalam Proses";

        String message = "Are you sure the vehicle with plate " + noPlat + " will be marked as in the washing process?";
        ...
        ModalDialog.showModal(this, new MessageModal(MessageModal.Type.INFO, message, title, SimpleModalBorder.YES_NO_OPTION,
            (controller, action) -> {
                if (action == SimpleModalBorder.YES_OPTION) {
                    ...
                    modelPesanan.setNoAntrian(model.getNoAntrian()); // Gunakan nomor antrian yang sudah ada
