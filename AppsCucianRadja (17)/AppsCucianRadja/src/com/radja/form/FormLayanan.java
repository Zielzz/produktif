...
import com.radja.form.input.FormInputLayanan;
...
import com.radja.tablemodel.TabModLayanan;
...
public class FormLayanan extends Form{

    private final TabModLayanan tblModel = new TabModLayanan();
    ...
    private void loadData(){
        ...
        List<Layanan> list = servis.getData(filter);
    ...
    private void searchData(){
        ...
        List<Layanan> list = servis.searchData(keyword, filter);
        tblModel.setData(list);
        tblData.setModel(tblModel);
    }
    ...
    private void insertData() {
        ...
        ModalDialog.showModal(this, new SimpleModalBorder(new FormInputLayanan(null, this), "Tambah Layanan"), option, "form input");
    }
    
    private void updateData() {
        int row = tblData.getSelectedRow();
        if (row != -1) {
            Layanan model = tblModel.getData(row);
            Option option = ModalDialog.createOption();
            option.setBackground(Color.BLACK);
            ModalDialog.showModal(this, 
                    new SimpleModalBorder(new FormInputLayanan(model, this), "Perbarui Layanan"), 
                    option, "form update");
        } else {
            ...
            ModalDialog.showModal(this, new MessageModal(MessageModal.Type.INFO, message, title, SimpleModalBorder.YES_NO_OPTION, 
                (controller, action) -> {
                    if (action == SimpleModalBorder.YES_OPTION) {
                        ...
                        modelLayanan.setDeleteBy(loggedInUser.getIdUser());

                        servis.deleteData(modelLayanan);
    ...
    private void viewDetail(){
        ...
        if (row != -1) {
            ...
            option.setBackground(Color.BLACK);
            ModalDialog.showModal(this, 
                    new SimpleModalBorder(new FormLayananDetail(idLayanan), "Detail"), 
                    option, "form detail");
        } else {
