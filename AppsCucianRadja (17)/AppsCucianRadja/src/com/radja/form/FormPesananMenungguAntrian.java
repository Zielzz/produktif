...
import com.radja.form.input.FormInputPesanan;
...
public class FormPesananMenungguAntrian extends Form{
    ...
    public FormPesananMenungguAntrian() {
        init();
    ...
    private JPanel setButton(){
        ...
        JButton btnAntri = new JButton("Tandai Antri");
        btnAntri.putClientProperty(FlatClientProperties.STYLE, ""
                + "foreground:@accentColor;"
                + "background:rgb(255,255,255);");
        btnAntri.setIcon(new FlatSVGIcon("com/radja/icon/edit.svg", 0.4f));
        btnAntri.setIconTextGap(5);
        btnAntri.addActionListener((e) -> updateData());
    ...
    private void hideColumnId(){
        ...
        tblData.getColumnModel().getColumn(8).setMinWidth(0);
        tblData.getColumnModel().getColumn(8).setMaxWidth(0);
        tblData.getColumnModel().getColumn(8).setWidth(0);
    }
    ...
    String status = "Menunggu Antrian";
    List<Pesanan> list = servis.getData(filter, status);
    // Sort by noAntrian
    list.sort((p1, p2) -> {
...
private void searchData() {
    ...
    String status = "Menunggu Antrian";
    List<Pesanan> list = servis.searchData(keyword, filter, status);
    // Sort by noAntrian
    list.sort((p1, p2) -> {
        Integer no1 = p1.getNoAntrian();
        Integer no2 = p2.getNoAntrian();
    ...
    private void insertData() {
        ...
        ModalDialog.showModal(this, new SimpleModalBorder(new FormInputPesanan(null, this), "Tambah Pesanan"), option, "form input");
    }
    ...
    if (row != -1) {
        ...
        String noPlat = model.getNomorPlat();
        String status = "Dalam Antrian";

        // Check if already in queue
        if ("Dalam Antrian".equals(model.getStatus())) {
            Toast.show(this, Toast.Type.INFO, "Pesanan sudah dalam antrian.", getOptionAlert());
            return;
        }

        if (loggedInUser == null) {
            Toast.show(this, Toast.Type.ERROR, "No user logged in.", getOptionAlert());
            return;
        }

        String message = "Are you sure the vehicle with plate " + noPlat + " will be marked as in queue?";
        ...
        ModalDialog.showModal(this, new MessageModal(MessageModal.Type.INFO, message, title, SimpleModalBorder.YES_NO_OPTION,
            (controller, action) -> {
                if (action == SimpleModalBorder.YES_OPTION) {
                    ...
                    modelPesanan.setStatus(status);
                    // Gunakan nomor antrian yang sudah ada
                    Integer noAntrian = model.getNoAntrian();
                    if (noAntrian == null) {
                        // Generate nomor antrian hanya jika belum ada
                        String noAntrianStr = servis.generateNoAntrian();
                        try {
                            noAntrian = Integer.parseInt(noAntrianStr);
                        } catch (NumberFormatException e) {
                            Toast.show(this, Toast.Type.ERROR, "Invalid queue number generated.", getOptionAlert());
                            return;
                        }
                    }
                    modelPesanan.setNoAntrian(noAntrian); // Set nomor antrian yang sudah ada atau baru
                    modelPesanan.setUpdateBy(loggedInUser.getIdUser());
                    modelPesanan.setIdPesanan(idPesanan);

                    try {
                        servis.updateData(modelPesanan);
                        Toast.show(this, Toast.Type.SUCCESS, "Data has been successfully marked in the queue.", getOptionAlert());
                        loadData();
                    } catch (RuntimeException e) {
                        Toast.show(this, Toast.Type.ERROR, "Failed to update data: " + e.getMessage(), getOptionAlert());
                    }
                }
            }), option);
    } else {
        Toast.show(this, Toast.Type.INFO, "Please select the data you want to mark in the queue", getOptionAlert());
    }
}
