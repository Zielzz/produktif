...
import com.radja.form.input.FormInputUser;
...
import com.radja.tablemodel.TabModUser;
...
public class FormUser extends Form{
    ...
    private final TabModUser tblModel = new TabModUser();
    ...
    private JPanel setInfo(){
        ...
        JLabel lbTitle = new JLabel("Data User");
    ...
    private void loadData(){
        ...
        List<User> list = servis.getData(filter);
    ...
    private void searchData(){
        ...
        List<User> list = servis.searchData(keyword, filter);
        tblModel.setData(list);
        tblData.setModel(tblModel);
    }
    ...
    private void insertData() {
        ...
        ModalDialog.showModal(this, new SimpleModalBorder(new FormInputUser(null, this), "Tambah User"), option, "form input");
    }
    
    private void updateData() {
        int row = tblData.getSelectedRow();
        if (row != -1) {
            User model = tblModel.getData(row);
            Option option = ModalDialog.createOption();
            option.setBackground(Color.BLACK);
            ModalDialog.showModal(this, 
                    new SimpleModalBorder(new FormInputUser(model, this), "Perbarui User"), 
                    option, "form update");
        } else {
            ...
            ModalDialog.showModal(this, new MessageModal(MessageModal.Type.INFO, message, title, SimpleModalBorder.YES_NO_OPTION, 
                (controller, action) -> {
                    if (action == SimpleModalBorder.YES_OPTION) {
                        ...
                        modelUser.setDeleteBy(loggedInUser.getIdUser());

                        servis.deleteData(modelUser);
